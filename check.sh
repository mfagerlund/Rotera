...
# --runInBand ensures sequential test execution to avoid race conditions with shared optimization state
npm test -- --passWithNoTests --silent --watchAll=false --runInBand > "$TEMP_DIR/frontend_test.log" 2>&1 &
FRONTEND_TEST_PID=$!
