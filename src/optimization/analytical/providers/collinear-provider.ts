 ...
 *
 * @param scale Optional scale factor (default 1.0). Same scale used by Line.computeResiduals (100.0).
 */
function createCollinearComponentProvider(
  ...
  return {
    ...
    computeGradient(variables: Float64Array): Float64Array {
      ...
      if (p0Map[0] >= 0) grad[p0Map[0]] = dp0.x * scale;
      if (p0Map[1] >= 0) grad[p0Map[1]] = dp0.y * scale;
      if (p0Map[2] >= 0) grad[p0Map[2]] = dp0.z * scale;

      if (p1Map[0] >= 0) grad[p1Map[0]] = dp1.x * scale;
      if (p1Map[1] >= 0) grad[p1Map[1]] = dp1.y * scale;
      if (p1Map[2] >= 0) grad[p1Map[2]] = dp1.z * scale;

      if (p2Map[0] >= 0) grad[p2Map[0]] = dp2.x * scale;
      if (p2Map[1] >= 0) grad[p2Map[1]] = dp2.y * scale;
      if (p2Map[2] >= 0) grad[p2Map[2]] = dp2.z * scale;
 ...
 *
 * @param scale Optional scale factor (default 1.0). For coincident points on lines, use 100.0 (GEOMETRIC_SCALE).
 */
export function createCollinearProviders(
  ...
  return [
    createCollinearComponentProvider(p0Indices, p1Indices, p2Indices, getP0, getP1, getP2, collinear_x_grad, scale),
    createCollinearComponentProvider(p0Indices, p1Indices, p2Indices, getP0, getP1, getP2, collinear_y_grad, scale),
    createCollinearComponentProvider(p0Indices, p1Indices, p2Indices, getP0, getP1, getP2, collinear_z_grad, scale),
  ];
}
