...
function inferFromLine(line: Line): { pointsUpdated: number; conflicts: InferenceConflict[] } {
  ...
  // even when we know the targetLength. The sign is always ambiguous - we don't
  // know if the target point is at +length or -length from the source point.
  // The optimizer must determine the sign based on image evidence.
