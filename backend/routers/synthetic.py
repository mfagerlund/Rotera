...
async def create_box_room(request: BoxRoomRequest) -> Dict[str, str]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create box room: {str(e)}")
...
async def create_grid_plane(request: GridPlaneRequest) -> Dict[str, str]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create grid plane: {str(e)}")
...
async def create_two_view(request: TwoViewRequest) -> Dict[str, str]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create two-view scene: {str(e)}")