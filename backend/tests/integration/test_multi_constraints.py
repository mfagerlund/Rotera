...
class TestMultiConstraintScenarios:
    ...
    def create_test_project(self):
        ...
        for wp_id, xyz in points.items():
            project.add_world_point(WorldPoint(
                id=wp_id,
                xyz=xyz
            ))
    ...
    def test_distance_and_axis_align(self):
        ...
        project.add_constraint(AxisAlignConstraint(
            wp_i="p1",
            wp_j="p2",
            axis="x"
        ))
        ...
        distance_factors = [f for f in factor_graph.factors.values()
                          if "distance" in f.factor_id]
        axis_factors = [f for f in factor_graph.factors.values()
                       if "axis_align" in f.factor_id]
        gauge_factors = [f for f in factor_graph.factors.values()
    ...
    def test_coplanar_with_known_coords(self):
        ...
        project.add_constraint(CoplanarConstraint(
            wp_ids=["p1", "p2", "p4", "p5"]
        ))
        ...
        known_coord_factors = [f for f in factor_graph.factors.values()
                              if "known_coord" in f.factor_id]
        coplanar_factors = [f for f in factor_graph.factors.values()
                           if "coplanar" in f.factor_id]
    ...
    def test_image_points_with_geometric_constraints(self):
        ...
        for wp_id, u, v in observations:
            project.add_constraint(ImagePointConstraint(
                image_id="img1",
                wp_id=wp_id,
                u=u,
                v=v,
        ...
        reprojection_factors = [f for f in factor_graph.factors.values()
                               if "reprojection" in f.factor_id]
        distance_factors = [f for f in factor_graph.factors.values()
                           if "distance" in f.factor_id]
        axis_factors = [f for f in factor_graph.factors.values()
                       if "axis_align" in f.factor_id]
    ...
    def test_conflicting_constraints_detection(self):
        ...
        project.add_constraint(DistanceConstraint(
            wp_i="p1",
            wp_j="p2",
            distance=2.0  # Conflicting distance!
        ))
    ...
    def test_collinear_with_distance_constraints(self):
        ...
        # Check constraint conversion
        collinear_factors = [f for f in factor_graph.factors.values()
                            if "collinear" in f.factor_id]
        distance_factors = [f for f in factor_graph.factors.values()
                           if "distance" in f.factor_id]
    ...
    def test_perpendicular_lines_constraint(self):
        ...
        project.add_constraint(PerpendicularConstraint(
            line1_wp_a="p1",
            line1_wp_b="p2",
            line2_wp_a="p4",
            line2_wp_b="p5"
        ...
        # Check constraint conversion
        perpendicular_factors = [f for f in factor_graph.factors.values()
                                if "perpendicular" in f.factor_id]
    ...
    def test_parallel_and_angle_constraints(self):
        ...
        from pictorigo.core.models.constraints import ParallelConstraint, AngleConstraint

        project = self.create_test_project()

        # Lines p1-p2 and p3-p4 should be parallel
        project.add_constraint(ParallelConstraint(
            line1_wp_a="p1",
            line1_wp_b="p2",
            line2_wp_a="p3",
            line2_wp_b="p4"
        ...
        project.add_constraint(AngleConstraint(
            line1_wp_a="p1",
            line1_wp_b="p2",
            line2_wp_a="p5",
            line2_wp_b="p6",
        ...
        # Check constraint conversion
        parallel_factors = [f for f in factor_graph.factors.values()
                           if "parallel" in f.factor_id]
        angle_factors = [f for f in factor_graph.factors.values()
                        if "angle" in f.factor_id]
    ...
    def test_complex_geometric_structure(self):
        ...
        from pictorigo.core.models.constraints import (
            CollinearConstraint, PerpendicularConstraint,
        ...
        project.add_constraint(FixedDistanceRatioConstraint(
            line1_wp_a="p1",
            line1_wp_b="p3",
            line2_wp_a="p1",
            line2_wp_b="p4",
        ...
        expected_types = {"collinear", "perpendicular", "distance_ratio", "distance", "gauge"}
    ...
    def test_all_constraint_types_together(self):
        ...
        from pictorigo.core.models.constraints import (
            CollinearConstraint, PerpendicularConstraint, ParallelConstraint,
            AngleConstraint, FixedDistanceRatioConstraint
        )
        ...
        points = {f"p{i}": [i * 0.5, (i % 3) * 0.5, (i % 2) * 0.5]
                 for i in range(1, 16)}
        ...
        project.add_constraint(ImagePointConstraint(
            image_id="img1", wp_id="p1", u=100, v=100
        ))
        ...
        project.add_constraint(DistanceConstraint(
            wp_i="p2", wp_j="p3", distance=1.0
        ))

        project.add_constraint(AxisAlignConstraint(
            wp_i="p4", wp_j="p5", axis="x"
        ))
        ...
        project.add_constraint(PerpendicularConstraint(
            line1_wp_a="p13", line1_wp_b="p14",
            line2_wp_a="p14", line2_wp_b="p15"
        ))

        project.add_constraint(ParallelConstraint(
            line1_wp_a="p2", line1_wp_b="p3",
            line2_wp_a="p4", line2_wp_b="p5"
        ))

        project.add_constraint(AngleConstraint(
            line1_wp_a="p6", line1_wp_b="p7",
            line2_wp_a="p8", line2_wp_b="p9",
            angle_degrees=90.0
        ))

        project.add_constraint(FixedDistanceRatioConstraint(
            line1_wp_a="p10", line1_wp_b="p11",
            line2_wp_a="p12", line2_wp_b="p13",
            ratio=1.5
        ))

        # Gauge fixing
        project.add_constraint(GaugeFixConstraint(
            origin_wp="p1", x_wp="p2", xy_wp="p4", scale_d=1.0
        ))
